...
Janus addresses this by deciding whether each candidate memory should be accepted or rejected. At step `t`, a base updater proposes a candidate memory $\hat{M}_t$. Janus either deploys it as the new memory $M_t$, or rolls back to the previous memory $M_{t-1}$. The goal is to maintain a final memory $M_T$ that is useful for future unseen tasks.
