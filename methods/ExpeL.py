...
class ExpeL(BaseMethod):
    ...
    _DEFAULT_SYSTEM_INSTRUCTION_PATH = _PROMPT_ROOT / "system_instruction.txt"
    _DEFAULT_HUMAN_INSTRUCTION_TEMPLATE_PATH = _PROMPT_ROOT / "human_instruction_template.txt"
    _DEFAULT_HUMAN_REFLECTION_TEMPLATE_PATH = _PROMPT_ROOT / "human_reflection_instruction_template.txt"
    _DEFAULT_TASK_TEMPLATE_PATH = _PROMPT_ROOT / "task_template.txt"
    _DEFAULT_RULE_TEMPLATE_PATH = _PROMPT_ROOT / "rule_template.txt"
    _DEFAULT_REFLECTION_PREFIX_PATH = _PROMPT_ROOT / "reflection_prefix.txt"
    _DEFAULT_REFLECTION_FEWSHOTS_PATH = _PROMPT_ROOT / "reflection_fewshots.txt"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # )
        # self._alfworld_human_reflection_template = self._read_prompt_text(
        #     self._DEFAULT_HUMAN_REFLECTION_TEMPLATE_PATH
        # )
        ...
        # self._alfworld_reflection_prefix = self._read_prompt_text(self._DEFAULT_REFLECTION_PREFIX_PATH)
        # self._alfworld_reflection_fewshots = self._read_reflection_fewshots(
        #     self._DEFAULT_REFLECTION_FEWSHOTS_PATH
        # )
    ...
    def _extract_single_action(self, output):
        ...
        for raw in source.splitlines():
            ...
            if not line:
                continue
            if re.match(
                r"^(go to|open|close|take|put|use|heat|cool|look|clean|inventory|examine|drop|slice)\b",
                line,
                flags=re.IGNORECASE,
            ):
                return line
    ...
    def _build_trial_model_output(self, step_rows, success):
        ...
        for row in step_rows:
            ...
            if observation:
                lines.append(observation)
        trajectory = "\n".join(lines).strip()
        final_answer = "SOLVED" if bool(success) else "UNSURE"
    ...
    def _normalize_rule_text(self, text):
        ...
        # Reject placeholder-only or non-informative rules such as:
        # "<rule>", "<rule>.", "rule", "rule.", "<strategy>".
        ...
        if lowered in {
            ...
            return ""
        # Very short templates like "<rule 1>" or "rule 1" are also useless.
        if re.fullmatch(r"<?\s*(rule|strategy|insight)\s*\d*\s*>?", lowered):
            return ""
    ...
    def _build_operation_format_instruction(self):
        return (
            ...
            "- Never output placeholder text such as '<rule>', '<strategy>', "
            "'rule', or template-like fillers.\n"
        )
    ...
    def _build_manual_fewshots_block(self, manual_fewshots, limit=2):
        shots = [str(s).strip() for s in (manual_fewshots or []) if str(s).strip()]
        if not shots:
            return "(none)"
        return "\n\n".join(shots[: max(1, int(limit))]).strip()

    def _build_reflection_fewshots_block(self):
        if not self._alfworld_reflection_fewshots:
            return "(none)"
        return "\n\n".join(self._alfworld_reflection_fewshots).strip()
    ...
    def _build_solver_prompt(
        ...
        if action_mode:
            ...
            manual_block = self._build_manual_fewshots_block(manual_fewshots, limit=2)
            system_text = self._alfworld_system_instruction
        ...
        human_text = (
            ...
            "Return exactly:\n"
            "<trajectory>concise step-by-step reasoning</trajectory>\n"
    ...
    def _build_reflection_prompt(self, prompt, failed_output, accumulated_reflection, action_mode=False):
        if action_mode:
            reflection_fewshots_block = self._build_reflection_fewshots_block()
            reflection_memory = self._format_reflection_memory(accumulated_reflection)
            return (
                f"{self._alfworld_human_reflection_template}\n"
                f"{reflection_fewshots_block}\n\n"
                "(END OF EXAMPLES)\n\n"
                f"{reflection_memory}"
                f"Previous trial:\n{prompt}\n{failed_output}\n"
                f"{self._alfworld_reflection_prefix}\n"
    ...
    def _extract_trajectory_and_answer(self, raw_output):
        raw = str(raw_output or "").strip()
        trajectory = self._extract_tag_content(raw, "trajectory")
        final_answer = self._extract_tag_content(raw, "final_answer")
        if not trajectory:
            trajectory = raw
        if not final_answer:
            final_answer = raw
    ...
    def run_trial(self, task, entry, inputs, timer, update_memory=True):
        ...
        for try_idx in range(1, self.max_tries + 1):
            ...
            for step_idx in range(max_steps):
                step_prompt = self._build_step_prompt_text(prompt, current_observation, step_rows)
                ...
                action = self._extract_single_action(raw_output)
                if not action:
                    break
            ...
            candidate_trajectory, candidate_answer = self._extract_trajectory_and_answer(candidate_raw)
    ...
    def generate(self, task, prompt, **kwargs):
        ...
        for try_idx in range(1, self.max_tries + 1):
            ...
            candidate_trajectory, candidate_answer = self._extract_trajectory_and_answer(candidate_raw)
            candidate_for_eval = str(candidate_raw) if action_mode else candidate_answer
    ...
    def update_memory(self, task, output, **kwargs):
        ...
        self._step_memories[task_name] = {
            "old_memory": insights_before,
            "new_memory": insights_after,
        }
        enriched = {
            "task": task_name,
            **record,
    ...
    def get_current_memory(self, task):
        return self._current_insights(task.name)

    def test_build_solver_prompt(self, task, entry, retrieved, inputs):
        # Frozen-insights solver prompt. Retrieved cases are kept because
        # they come from per-question embedding search and vary per
        # sample -- this is explicitly allowed even in frozen-memory test.
        prompt = task.build_prompt(entry)
        insights = self._current_insights(task.name)
        inputs = inputs if isinstance(inputs, dict) else {}
        return self._build_solver_prompt(
            prompt=prompt,
            insights=insights,
    ...
    def restore_state_from_memory(self, task):
        ...
        self._task_rule_items_with_count[task_name] = []
        self._task_recent_success[task_name] = []
        self._task_experience_pool[task_name] = []
        self._task_retrieval_docs[task_name] = []
        self._task_retrieval_vectors[task_name] = []
        self._task_pending_rollout[task_name] = {}
        self._task_seeded[task_name] = True

        last_memory = task_records[-1].get("memory", {})
        insights_text = ""
        if isinstance(last_memory, dict):
            insights_text = str(last_memory.get("insights", "")).strip()
        elif isinstance(last_memory, str):
            insights_text = str(last_memory).strip()
        ...
        for raw_line in insights_text.splitlines():
            ...
            if not line:
                continue
            line = re.sub(r"^\d+\.\s*", "", line).strip()
            normalized = self._normalize_rule_text(line)
            if not normalized:
                continue
            restored_rules.append((normalized, 2))
        self._task_rule_items_with_count[task_name] = restored_rules

        restored_success = 0
        for rec in task_records:
            if str(rec.get("feedback", "")).lower() != "success":
                continue
            question = str(rec.get("question", "")).strip()
            if not question:
                continue
            memory_block = rec.get("memory", {})
            rollout = memory_block.get("rollout", {}) if isinstance(memory_block, dict) else {}
            attempts = rollout.get("attempts", []) if isinstance(rollout, dict) else []
            reflections = rollout.get("reflections", []) if isinstance(rollout, dict) else []

            trajectory = str(rec.get("model_output", "")).strip()
            score = rec.get("score")
            for attempt in attempts:
                if attempt.get("feedback") == "success":
                    trajectory = str(
                        attempt.get("trajectory", attempt.get("model_output", trajectory))
                    ).strip()
                    score = attempt.get("score", score)
                    break

            success_case = {
                "question": question,
                "model_output": str(rec.get("model_output", "")).strip(),
                "trajectory": trajectory,
                "score": score,
                "reflections": [str(x).strip() for x in reflections if str(x).strip()],
            }
            self._append_success_case(task_name, success_case)
            restored_success += 1

        print(
            f"[ExpeL Resume] Restored {restored_success} success cases, "
            f"rules={len(restored_rules)}"
        )
    ...
    def get_step_memories(self, task):
        return self._step_memories.get(
            task.name,
            {
                "old_memory": self._current_insights(task.name),
                "new_memory": None,
            },
        )