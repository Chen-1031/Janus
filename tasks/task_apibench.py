...
class APIBenchBase(BaseTask):
    ...
    def _phase_offset(self) -> Optional[int]:
        """Where to start in the seeded-shuffled eval pool.

        train -> [0 : sample_size); test -> [sample_size : 2*sample_size);
        full / all / raw -> use every row (no slicing).
        """
        ...
        if self.split in {"full", "all", "raw", ""}:
            return None
        raise ValueError(
            "Unsupported APIBench split. Use train, test, or full. "
            "train/test produce two deterministic disjoint slices of size "
            "--train-sample-size (default 250), seeded by --sample-seed."
        )
    ...
    def _stable_shuffle(
        ...
        digest = hashlib.sha256(
            f"APIBench:{self._subset}:{self.sample_seed}".encode("utf-8")
        ).hexdigest()
    ...
    def _load(self) -> None:
        ...
        if not eval_path.exists():
            raise RuntimeError(
                ...
                f"Run: python data/APIBench/download.py"
            )
        ...
        if offset is not None:
            ...
            if len(entries) < need:
                raise RuntimeError(
                    f"APIBench subset={self._subset!r} has only "
                    f"{len(entries)} rows, but split={self.split!r} with "
                    f"sample_size={self.sample_size} requires at least {need}."
                )
            shuffled = self._stable_shuffle(entries)
            entries = shuffled[offset : offset + self.sample_size]
